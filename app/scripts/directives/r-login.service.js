(function () {
  ...
  angular.module('r3plicaApp.directives').service('rLoginService', service);

  function service() {
    var email = document.querySelector('#email'),
      password = document.querySelector('#password'),
      mySVG = document.querySelector('.svgContainer'),
      armL = SVG.select('.armL'),
      armR = SVG.select('.armR'),
      eyeL = SVG.select('.eyeL'),
      eyeR = SVG.select('.eyeR'),
      nose = SVG.select('.nose'),
      mouth = SVG.select('.mouth'),
      mouthBG = SVG.select('.mouthBG'),
      mouthSmallBG = SVG.select('.mouthSmallBG'),
      mouthMediumBG = SVG.select('.mouthMediumBG'),
      mouthLargeBG = SVG.select('.mouthLargeBG'),
      mouthMaskPath = SVG.select('#mouthMaskPath'),
      mouthOutline = SVG.select('.mouthOutline'),
      tooth = SVG.select('.tooth'),
      tongue = SVG.select('.tongue'),
      chin = SVG.select('.chin'),
      face = SVG.select('.face'),
      eyebrow = SVG.select('.eyebrow'),
      outerEarL = SVG.select('.earL .outerEar'),
      outerEarR = SVG.select('.earR .outerEar'),
      earHairL = SVG.select('.earL .earHair'),
      earHairR = SVG.select('.earR .earHair'),
      hair = SVG.select('.hair');
    ...
    function getCoord(e) {
      ...
      console.log('eyeLAngle', eyeLAngle);
      console.log('eyeLX', eyeLX);

      eyeL.move(-eyeLX, -eyeLY);
      eyeR.move(-eyeRX, -eyeRY);
      nose.move(-noseX, -noseY);
